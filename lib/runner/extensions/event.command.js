    ...
    process: {
        ...
        event (payload, next) {
            var item = payload.item,
                ...
                requireResolver = this.options.requireResolver,
            ...
            async.mapSeries(events, function (event, next) {
                ...
                const executeScript = (requireFileResolver) => {
                ...
                const path = [item.id];
                item.forEachParent({ withRoot: true }, (item) => path.push(item.id))

                // TODO: Instead of just sending script.packages, we should be
                // sending a script object with only required things:
                // {
                //   id: script.id,
                //   packages: script.packages
                // }
                requireResolver(script.packages, (err, requireFileResolver) => {
                    if (err || !requireFileResolver) {
                        return next(err);
                    }

                    executeScript(requireFileResolver);
                });
